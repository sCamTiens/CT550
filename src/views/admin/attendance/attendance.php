...
<div x-data="attendancePage()" x-init="init()">
    <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            <div class="relative" @click.away="filterModeOpen=false">
                <button type="button"
                    @click="filterModeOpen = !filterModeOpen"
            ...
            <template x-if="filterMode === 'week'">
                <div class="flex gap-2">
                    ...
                    <div class="relative" @click.away="yearOpen=false">
                        ...
                        <ul x-show="yearOpen"
                            class="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-auto">
                            <template x-for="y in Array.from({ length: new Date().getFullYear() - 2020 + 1 }, (_, i) => 2020 + i).reverse()" :key="y">
                ...
                <table style="width:100%; border-collapse:collapse;">
                    <thead class="bg-[#002975] text-white">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left">Ca làm việc</th>
                            <th class="px-4 py-3 text-left">Ngày</th>
                            <th class="px-4 py-3 text-left">Giờ vào</th>
                            <th class="px-4 py-3 text-left">Giờ ra</th>
                            <th class="px-4 py-3 text-left">Trạng thái vào</th>
                            <th class="px-4 py-3 text-left">Trạng thái ra</th>
                    ...
                    <tbody>
                        <template x-for="(item, idx) in paginatedItems()" :key="item.id">
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3" x-text="formatDate(item.attendance_date)"></td>
                                <td class="px-4 py-3" x-text="formatDateTime(item.check_in_time)"></td>
                                <td class="px-4 py-3" x-text="formatDateTime(item.check_out_time) || '-'"></td>
                                <td class="px-4 py-3">
    ...
    <div class="flex items-center justify-center mt-4 gap-6">
        <div class="text-sm text-slate-600">
            Tổng cộng <span x-text="items.length"></span> bản ghi
        </div>
