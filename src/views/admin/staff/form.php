...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <!-- Vai trò -->
    <div>
        <label class="block text-sm text-black font-semibold mb-1">Vai trò <span class="text-red-500">*</span></label>
        <select x-model="form.staff_role" class="border rounded px-3 py-2 w-full text-gray-700" required
            @blur="touched.staff_role = true; validateField('staff_role')"
            @change="touched.staff_role && validateField('staff_role')">
            <option value="">-- Chọn vai trò --</option>
            <template x-for="role in staffRoles" :key="role.name">
...
<script>
    ...
    <template x-if="!form.user_id">
        <div>
            ...
            <p x-show="touched.password_confirm && errors.password_confirm" x-text="errors.password_confirm" class="text-red-500 text-xs mt-1"></p>
        </div>
    </template>
